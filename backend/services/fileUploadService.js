...
export const fileUploadService = {
  ...
  async uploadAvatarImage(file, userId) {
    ...
    const mimeOk =
      ...
      ['image/pjpeg', 'image/x-png'].includes(file.mimetype);
    ...
    if (file.size > maxAvatarSize) {
      throw new Error('Image too large. Max size: 2MB');
    }

    const fileExt = extOk ? ext : 'jpg';
    ...
    if (error) {
      logger.error('Avatar upload error', { error: error.message, userId, filePath });
  ...
  extractStoragePath(publicUrl) {
    ...
    const marker = '/storage/v1/object/public/booknest/';
