...
async getBooksByUser(userId, page = 1, limit = 20) {
  try {
    ...
    const bookSelectBase = bookSelectWithReview
      .replace('review_note,\n        reviewed_at,\n        ', '');

    let { data: books, error: booksError } = await supabaseAdmin
      .from('books')
      .select(bookSelectWithReview)
      .eq('uploaded_by', userId)
      .order('created_at', { ascending: false })
      .range(from, to);

    if (booksError?.message?.includes('review_note')) {
      ({ data: books, error: booksError } = await supabaseAdmin
        .from('books')
        .select(bookSelectBase)
        .eq('uploaded_by', userId)
        .order('created_at', { ascending: false })
        .range(from, to));
    }
