...
async updateBookCover(bookId, userId, coverImagePath, coverImageUrl) {
 ...
 async findMany({ genreId, format, search, page = 1, limit = 12, userId = null }) {
  try {
    ...
    // Apply status filter (only approved for guests)
    if (!userId) {
...
async findById(bookId, userId = null) {
  try {
    ...
    // If user is not logged in, only show approved books
    if (!userId) {
      query = query.eq('status', 'approved');
    }
    ...
    // Get formats (always show formats for approved books)
    const { data: formats, error: formatsError } = await supabaseAdmin
      .from('book_formats')
      .select('id, format_type, price, currency, page_count, duration_sec, file_size_bytes, storage_path')
      .eq('book_id', bookId);
