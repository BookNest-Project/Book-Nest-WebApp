...
export const authService = {
  ...
  async login(email, password) {
    ...
    switch (dbUser.role) {
      ...
      case 'admin': {
        ...
        const { data: authData } = await supabaseAdmin.auth.admin.getUserById(authUser.id);
        authMeta = authData?.user?.user_metadata || null;
        break;
      }
    }
    ...
    return {
      ...
      session: createAuthSession(dbUser, profile, authMeta),
    };
  },
  ...
  async getUserSession(userId) {
    ...
    return createAuthSession(dbUser, profile, authMeta);
  },
